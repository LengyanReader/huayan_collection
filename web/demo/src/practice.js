...
window.heartLogin=function(){
  ...
  .then(function(u){
    if(u.login==='LengyanReader'){
      ...
      localStorage.setItem('gh_username','LengyanReader');
      _update();_toast('✅ LengyanReader · Push已启用');
    }else{
      _toast('⛔ 仅限LengyanReader账号 · 当前Token属于'+u.login);
      localStorage.removeItem(STORAGE_KEY);ghToken=null;
    }
