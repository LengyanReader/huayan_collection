...
var COSMO_WINDS=(typeof COSMO_DATA!=='undefined'&&COSMO_DATA.cosmo_layers&&COSMO_DATA.cosmo_layers.winds)?COSMO_DATA.cosmo_layers.winds:[];
var COSMO_LAYERS=(typeof COSMO_DATA!=='undefined'&&COSMO_DATA.cosmo_layers&&COSMO_DATA.cosmo_layers.layers)?COSMO_DATA.cosmo_layers.layers:[];
...
function drawTower(){
  ...
  // ── Layout columns (from COSMO_DATA.three_realms) ──
  var _tr=(typeof COSMO_DATA!=='undefined'&&COSMO_DATA.three_realms)?COSMO_DATA.three_realms:{};
  var _cols=_tr.columns||{};
  var colX={practice:(_cols.practice||{}).x||10,heaven:(_cols.heaven||{}).x||140,bar:(_cols.bar||{}).x||280,stage:(_cols.stage||{}).x||560,method:(_cols.method||{}).x||680};
  var colW={practice:(_cols.practice||{}).w||120,heaven:(_cols.heaven||{}).w||40,bar:(_cols.bar||{}).w||260,stage:(_cols.stage||{}).w||120,method:(_cols.method||{}).w||180};
  var top=40,bot=30,avail=H-top-bot;
  var data=_tr.realms||[];

  // ── Column headers ──
  ctx.fillStyle='#b8863c';ctx.font='bold 12px Microsoft YaHei';
  ctx.fillText((_cols.practice||{}).label||'修行次第(海云法师体系)',colX.practice,top-12);
  ctx.fillText((_cols.heaven||{}).label||'二十八天',colX.heaven,top-12);
  ctx.fillText((_cols.stage||{}).label||'十信位·果位',colX.stage,top-12);
  ctx.fillText((_cols.method||{}).label||'禅定进路',colX.method,top-12);
  ...
  // ── Legend at bottom ──
  var lx=8,ly=H-18;
  (_tr.legend||[{n:'无色界(妙行)',c:'#8b7a9e'},{n:'色界(正行)',c:'#5e8b9e'},{n:'欲界(前行)',c:'#c46b5d'},{n:'人间·恶道',c:'#b8863c'}]).forEach(function(lg){
  ...
  ctx.fillStyle='#a09080';ctx.font='8px Microsoft YaHei';ctx.fillText(_tr.source||'源: 大华严寺官网·华严禅观全程一览表 + 海云继梦《四十华严讲记》',lx+10,ly+1);
}
