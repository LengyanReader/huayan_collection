...
window.renderComments=function(tab){
  ...
  h+='<div class=c-list>';cs.slice(-8).forEach(function(c,i){var idx=cs.length-8+i;if(idx<0)idx=0;
    ...
    var ct=c.t;
    // Direct image detection: if text contains data:image link, render as img
    var di=ct.indexOf('](data:image/');
    if(di>=0){
      var fi=ct.lastIndexOf('![',di);
      if(fi>=0){
        var alt=ct.substring(fi+2,ct.indexOf(']',fi));
        var uriStart=di+2;
        var uriEnd=ct.indexOf(')',uriStart);
        if(uriEnd<0)uriEnd=ct.length;
        var uri=ct.substring(uriStart,uriEnd);
        ct=ct.substring(0,fi)+'<div style=text-align:center;margin:6px 0><img src="'+uri+'" alt="'+alt+'" style=max-width:200px;border-radius:6px;box-shadow:0 1px 4px rgba(0,0,0,0.1)></div>'+ct.substring(uriEnd+1);
      }
    }
