...
function selectPerson(id,isShift,ev){
  ...
  if(mapMain&&locs.length>0){
    ...
    setTimeout(function(){
      mapMini.eachLayer(function(layer){if(!layer._ld)return;var mc={temple:"#b8863c",mountain:"#7d9a6e",region:"#c46b5d"};layer.setRadius(8);layer.setStyle({fillColor:mc[layer._ld.tp]||"#b0a898",color:"#fff",weight:2,fillOpacity:0.9});});
    },4000);
  }
...
var ANIM_WAYPOINTS=[
  ...
  {y:1991,lat:25,lng:121.5,z:7,label:'梦参剃度·临济47代·台北',info:'海云继梦(陈鹤山)于梦参老和尚座下剃度,法名昌一号继梦。确立临济宗第47代传人。此前十年(1981-1991)以在家身于台北多地弘扬华严,奠定「以现代语言诠释华严古义」的教学风格。'},
  {y:2004,lat:34.2,lng:108.9,z:6,label:'西安弘法·学术建制化',info:'受聘陕师大华严研究所荣誉所长。中国社会科学院演讲(首位登台出家人)。2006-2010北京广化寺系统弘讲四十华严全本。义学体系的学术建制化由此展开。'},
  {y:2008,lat:23.9,lng:120.9,z:9,label:'三脉汇流·普贤乘华严宗确立·南投',info:'★核心年份★ ①钦因传华严衣钵(贤首42世) ②印度胜师子王菩萨传瑜伽行法 ③华严·临济·瑜伽行三脉归一。提出「普贤乘华严宗」定位 +「东山法门等持工程」修行体系。核心技术:技术面(数随止三法)+工程面(A/B/C心法)双轨教学。四阶段蓝图:出离→菩提→菩萨→普贤乘。'},
  {y:2011,lat:25,lng:121.5,z:7,label:'华严六科·五大行法·三部曲定型',info:'华严六科(概论/经教/禅法/戒律/净土/密法) + 五大行法(禅净律密经教)体系完整出版。三部曲(非常坛经/金刚经/心经)确立修行理论框架。密集出版期:2011-2013完成框架定型。'},
  {y:2013,lat:25,lng:121.5,z:7,label:'华严研究所·学报·制度化',info:'海云华严研究所成立,熊琬教授任所长。华严学报创刊(已出14期)。忍可灌顶制度+禅观全程一览表公开发布。修学体系从个人教学转向制度化的僧团教育。'},
  {y:2023,lat:25,lng:121.5,z:7,label:'台北大学杰出校友·学术合作',info:'获选国立台北大学杰出校友。2024年华严学会与台北大学签署学术合作协议(一期两年),合办永续发展国际研讨会。修行体系与主流学术机构正式对接。'},
  {y:2026,lat:24.5,lng:120.7,z:9,label:'九九华严·支提山·第四期佛教',info:'★核心年份★75岁高龄。①台北TICC启动五年讲座「九九华严」,将华严哲理转化为现代心理学与生命科学语言,回应AI时代挑战。②苗栗通霄支提山大华严寺动土(7月9日),面向台湾海峡与福建宁德支提华严祖庭隔海相望。五十三位嘉宾执铲呼应善财五十三参。③提出「第四期佛教」自我定位——义理传播深化+道场空间扩展双线并行。'}
  {y:2021,lat:27.7,lng:85.3,z:5,label:'84000英译·华严回归国际',info:'Peter Alan Roberts英译入法界品(Toh44-45)出版。藏文华严首次系统英译,华严学研究进入多语对读新时代。'},
  {y:2026,lat:24.5,lng:120.7,z:9,label:'九九华严·支提山动土·苗栗',info:'于台北TICC启动五年讲座(每月举办)。苗栗通霄支提山大华严寺动土(7月9日),面向台湾海峡与福建宁德支提华严祖庭隔海相望。五十三位嘉宾执铲呼应善财五十三参。'}
];
var lastAnimLoc=-1;
...
function toggleAnim(){
  ...
  if(mapMini){
    mapMini.setView([34,110],5);
    animRouteLineU=L.polyline(routeCoords,{color:'#b8863c',weight:2,opacity:0.35,dashArray:'6,8'}).addTo(mapMini);
    animRouteMarkerU=L.circleMarker([34,110],{radius:6,fillColor:'#b8863c',color:'#ff6',weight:2,fillOpacity:0.9}).addTo(mapMini);
  }
  ...
  function animStep(){
    ...
    animYear+=sp;
    if(animYear>2030){
      animYear=-600;clearInterval(animTimer);animPlaying=false;lastAnimLoc=-1;
      document.getElementById("anim-btn").textContent="▶ 播放";
      [animRouteLineM,animRouteMarkerM,animRouteLineU,animRouteMarkerU].forEach(function(l){if(l&&mapMain)mapMain.removeLayer(l);if(l&&mapMini)mapMini.removeLayer(l);});
      animRouteLineM=animRouteMarkerM=animRouteLineU=animRouteMarkerU=null;
      if(speedLabel)speedLabel.textContent='1×';
    ...
    // Find active waypoint (last one before current year)
