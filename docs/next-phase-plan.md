...
### L-E 多语对读文章《识与心灵》《识与心灵之镜》（P0 进行中 2026-08-30）

**目标**：两篇项目作者自有研究长文（唯识学×当代意识科学 / 唯识学×西方心理学）以「中文为主文·外文为平行对读批注」形态整合为独立文章页，接入前沿对话 Tab「与佛教的对话」→「瑜伽行派 & 华严」域。

- 设计规范：`docs/bilingual-annotation-design.md`（呈现模型/批注格式/语言取舍/来源分级/路线图）
- P0：文章页骨架 + 摘要/关键词/核心术语批注示范 + 后端接入；**已完成**（前后端、注册、出处登记、构建验证）
- P1：全节「英文无损对应」铺满全部节（以作者 04-15 英文修订版为底本；对中文每节内容给出**信息无损**的英文对应，简明优雅、直白不铺陈，而非仅摘句）
- P2：术语格义表覆盖正文全部核心唯识术语（梵/藏/英）
...
- 落地序列（自 L-E 起）：
  1. L-E 两篇多语对读文章：从「节级英译要义(节首原句)」升级为「全节英文无损对应」，覆盖每一节全部内容块（含漫谈/案例/表格说明）；
  2. 前沿对话 Tab 的域卡片与各节 intro：YAML 增 `en` 字段，域名/核心议题/关键发现给出英文对应；
  3. 全站数据 YAML/SQLite 逐表增 `en` 字段 → build.py 渲染中英对照 → 全局「仅中文/中英对照」阅读切换；
