...
| 维度 | 数据 |
|------|------|
| 人物 | **85人**（印度源流+华严五祖+高原法系+华严莲社+月霞系+日本华严+高丽华严+临济宗+译师+学者+参考线） |
| 传承边 | **81条**（MASTER/INFLUENCED/LINEAGE/CONTEMPORARY） |
| 地点 | 23处（含大华严寺/福慧寺/东大寺/支提山/那烂陀等） |
| 历史事件 | 44条（远源/宗教/义学/翻译/禅观/宗派/文化/法难/教育/传承/当代） |
| 动画节点 | 85个（吠陀→奥义书→释迦生平→佛教东传→华严宗→近现代→2026） |
| Tab数 | **5个** |
| 构建 | 单文件 ~237KB → 目标: 6个HTML (~350KB总计) |
...
| 方向 | 内容 | 状态 |
|------|------|------|
| **多页面拆分** | 单HTML→6个HTML(index+5tabs) | 🔴 待实施 |
| **共享CSS/JS提取** | common.css+common.js | 🔴 待实施 |
| **Tab2布局重构** | 华严文献 sidebar+content双栏 | 🔴 待实施 |
| **Tab3布局重构+更名** | 华严行法→华严教行，sidebar双栏 | 🔴 待实施 |
| **Tab4布局重构** | 前沿对话 sidebar双栏 | 🔴 待实施 |
| **Tab5布局重构** | 世主妙严 sidebar双栏 | 🔴 待实施 |
| **cosmology.js数据→YAML** | COSMO_LAYERS等→data/cosmology/*.yaml | 🔴 待实施 |
| **gap.js数据→YAML** | 术语/案例/观点→glossary表+gap_content.yaml | 🔴 待实施 |
| **practice.js数据→YAML** | ~660行→data/practice/*.yaml | 🔴 待实施 |
