...
### 多语 EN 翻译管线（翻译与全文校对结合，不偏废）

原则：**以文本质量为优先对象，在线翻译（API）仅作上量辅助，绝不替代人工/主编全文审查。** 多语对应重在「实质内容（义·理·境）准确对应」，不逐字硬译。

已落地批次（Tab3 华严教行 ⑥-⑬ 全量 EN）执行的是这条「校对耦合」工作流，而非纯 API 直译：

1. **规则先行**：全站固定译法表（CLAUDE.md 已列：五种姓/三性三无性/八识/十玄/华严三昧/五教止观/……）作为约束输入，保证跨文件专名一致。
2. **初稿**：可由辅助 API（GLM-4-Flash 免费档）批量生成 EN 候选稿，也可由子代理（模型）著写——当前批次实际用子代理完成，兼顾教理与文风。
3. **机械门禁**（脚本）：YAML 可解析 + `_en` 字段齐全 + 引号/撇号/块标量安全检查（`'`坑、双引号内嵌引号破坏）。
4. **主编全文审查**：主模型逐文件复核——译名一致性对照术语表、正式用词（学界/佛界通行译名，梵文转写优先）、教理准确性、考证优先（〔待核〕/〔存疑〕标注、严禁编造史实/臆造译名/无对应时硬译充数，无通行英译须明示标注并保留原文）、意译而非逐字。
5. **渲染/构建验证**：build.py + verify_demo.py ALL PASSED + built 标记抽查命中。

辅助 API 接入为**可插拔**：有 `ZHIPU_API_KEY` 时第一步调用 GLM-4-Flash 上量，无 key 自动回退子代理著写；质量门禁与主编审查始终保留，不因接入 API 而放松。**PoC 待做**：以已翻译文件回测「术语表约束→初稿→门禁→审查」token 节省比后再推广。
