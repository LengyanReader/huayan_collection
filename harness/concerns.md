...
| 落于 | 现有资产 | 〔缺口〕 |
|---|---|---|
| 主 C-D | L.㊽ **文体凡例八条**（章节落事实不收格言、加粗只留表格、段落参差、不用"首先/其次/最后"套话）· L.㊾ 语域学术化 + 文言残留归零 · 观点溯源七类标注（推论/评估/论证/主张/预测/立场/总结 + 逐条注依据）· `zh-academic-register` 技能 · GB/T 15835 数字规范 · 加粗密度指标（曾 10.97→9.50 每千字）· `citation-verification` · 论断强度校准（"决定"→"自然体现"） | [x] **已纳入 rules.md §I**（2026-09-26：文体凡例八条+观点溯源七类+论断强度校准已升格为常设规矩）<br>[~] **自动检查部分落地**：`scripts/audit_style.py` v1（报告模式）已扫 加粗密度(I2)+套话(I4)，基线 31 篇/超标 20/含套话 15（头号拐杖词“值得注意的是”）；文言残留(I5)+CJK-in-EN 仍〔启发式待细化〕<br>[ ] **EN 端文体规范**缺（中文有凡例，英文无）<br>[ ] 反幻觉的"不表演式坦白/不虚构细节"仅散在批次记录 |

**建议路径**：①把文体凡例八条 + 观点溯源七类 + 论断强度校准并入 `rules.md §I 行文与文体`；②建 `scripts/audit_style.py` 扫描套话/加粗密度/文言残留；③用 `zh-academic-register` 技能承接改写工作。

---

## §4 合规 · 版权 · 伦理 (Compliance · Copyright · Ethics)　✅ 已建登记面 [`compliance.md`](compliance.md)（2026-09-26）
...
**现状**：已依维护者四项立场（公开可得 · 非商用 · 合理使用 · 学术梳理）建 [`compliance.md`](compliance.md)，登记立场(S1–S5)/决策(D1–D4)/各源权利分层表/残留风险清单(R1–R8)。**决策=保持现状**（不改根 LICENSE、不动 1292 讲记文件），README 已加权利披露。**〔残留待触发复审〕**：CBETA/84000 现行条款逐条对读(R3)·图档逐件许可(R4)·微信抓取改摘要+回源(R6)·真伪表述具名出处规则(R7)——见 compliance.md §四。
